...
= [stixtitle]*STIX^TM^ 2.2 Incident Expansion*
...
In this 2.2 proposal we add additional core properties to the Incident object and also add additional event, impact, and task objects. This also makes published optional for reports to allow easier MISP integration and to better handle incident reports that are in a pre-publication state.
...
Incident objects represent cases composed of events and tasks as well as actual or potential impacts. An Incident can be created prior to a formal determination that the incident has an impact as a way to logically track case work in an attempt to investigate events or lower level alerts.
...
Some automated tools may flag results as [stixliteral]#blocked# or [stixliteral]#false-positive# automatically depending on the tool type or activity.
...
|*type* (required)
|[stixtype]#{string_url}[string]#
|The value of this property *MUST* be set to [stixliteral]#incident#.
...
|
|
|Converting to STIX 2.2 proposal
